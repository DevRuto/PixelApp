...
(function (Facepunch) {
  const Loader = (function () {
    ...
    Loader.prototype.getLoadProgress = function () {
      const total = this.queue.length + this.active.length + this.completed;
      let complete = this.completed;
      for (let _i = 0, _a = this.queue; _i < _a.length; _i++) {
        var item = _a[_i];
        complete += item.getLoadProgress();
      }
      for (let _b = 0, _c = this.active; _b < _c.length; _b++) {
        var item = _c[_b];
        complete += item.getLoadProgress();
      }
    ...
    Loader.prototype.getNextToLoad = function () {
      ...
      let bestPriority = 0;
      for (let i = 0, iEnd = this.queue.length; i < iEnd; ++i) {
    ...
    Loader.prototype.update = function (requestQuota) {
      ...
      const _loop_1 = function () {
        ...
        next.loadNext(function (requeue) {
          ...
          if (requeue) { _this.queue.push(nextCopy); } else { ++_this.completed; }
  ...
  const _LZString = (function () {
    ...
    var _LZString = {
      compressToBase64 (input) {
        ...
        const res = _LZString._compress(input, 6, function (a) { return keyStrBase64.charAt(a); });
        switch (res.length % 4) {
      ...
      compressToUint8Array (uncompressed) {
        const compressed = _LZString.compress(uncompressed);
      ...
      _compress (uncompressed, bitsPerChar, getCharFromInt) {
        ...
        let i; let value; const context_dictionary = {}; const context_dictionaryToCreate = {}; let context_c = ''; let context_wc = ''; let context_w = ''; let context_enlargeIn = 2; // Compensate for the first entry which should not count
        let context_dictSize = 3; let context_numBits = 2; const context_data = []; let context_data_val = 0; let context_data_position = 0; let ii;
      ...
      _decompress (length, resetValue, getNextValue) {
        const dictionary = []; let next; let enlargeIn = 4; let dictSize = 4; let numBits = 3; let entry = ''; const result = []; let i; let w; let bits; let resb; let maxpower; let power; let c; const data = { val: getNextValue(0), position: resetValue, index: 1 };
        for (i = 0; i < 3; i += 1) {
          dictionary[i] = i;
        }
        bits = 0;
...
(function (Facepunch) {
  ...
  const Vector3 = (function () {
    ...
    Vector3.prototype.applyQuaternion = function (quat) {
      ...
      const x = this.x; const y = this.y; const z = this.z;
      const qx = quat.x; const qy = quat.y; const qz = quat.z; const qw = quat.w;
  ...
  const Quaternion = (function () {
    ...
    Quaternion.prototype.setNormalized = function (quat) {
      if (quat === undefined) { quat = this; }
      const len = Math.sqrt(quat.x * quat.x + quat.y * quat.y +
    ...
    Quaternion.prototype.multiply = function (quat) {
      ...
      const qbx = quat.x; const qby = quat.y; const qbz = quat.z; const qbw = quat.w;
  ...
  const Matrix4 = (function () {
    ...
    Matrix4.prototype.setPerspective = function (fov, aspect, near, far) {
      const top = near * Math.tan(0.5 * fov); const height = 2 * top; const width = aspect * height; const left = -0.5 * width; const right = left + width; const bottom = -top;
      // From https://github.com/mrdoob/three.js
      const m = this.elements;
...
(function (Facepunch) {
  ...
  Http.cancelled = { toString () { return 'Request cancelled by user.'; } };
  Facepunch.Http = Http;
  ...
  const WebGl = (function () {
    ...
    WebGl.encodeConst = function (value) {
      if (WebGl.constDict == null) {
        WebGl.constDict = {};
        for (const name_1 in WebGLRenderingContext) {
          const val = WebGLRenderingContext[name_1];
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    ...
    const CommandBufferParameter = (function () {
    ...
    const CommandBuffer = (function () {
      ...
      CommandBuffer.prototype.getCommandName = function (action) {
        for (const name_2 in this) {
          if (this[name_2] === action) { return name_2; }
        }
        return undefined;
      };
      CommandBuffer.prototype.logCommands = function () {
        ...
        for (let i = 0, iEnd = this.commands.length; i < iEnd; ++i) {
          ...
          for (const name_3 in command) {
            ...
            if (value !== undefined) { params.push(name_3 + ': ' + value); }
          }
      ...
      CommandBuffer.prototype.run = function () {
        ...
        if (this.tempLastRunTime !== undefined && (time - this.tempLastRunTime) < 1000.0) {
          ...
          const frames_1 = Math.floor(this.tempSpareTime * 60.0 / 1000.0);
      ...
      CommandBuffer.prototype.push = function (action, args) {
        if (this.immediate) { throw new Error('CommandBuffer.push was called in immediate mode!'); } else {
          args.action = action;
      ...
      CommandBuffer.prototype.clear = function (mask) {
        if (this.immediate) { this.context.clear(mask); } else { this.push(this.onClear, { mask }); }
      };
      ...
      CommandBuffer.prototype.dynamicMaterial = function (callback) {
        ...
        this.push(function (gl, args) {
          const wasImmediate = _this.immediate;
      ...
      CommandBuffer.prototype.setCap = function (cap, enabled) {
        ...
        if (this.immediate) {
          if (enabled) { this.context.enable(cap); } else { this.context.disable(cap); }
        } else { this.push(enabled ? this.onEnable : this.onDisable, { cap }); }
      };
      ...
      CommandBuffer.prototype.depthMask = function (flag) {
        ...
        if (this.immediate) { this.context.depthMask(flag); } else { this.push(this.onDepthMask, { enabled: flag }); }
      };
      ...
      CommandBuffer.prototype.blendFuncSeparate = function (srcRgb, dstRgb, srcAlpha, dstAlpha) {
        if (this.immediate) { this.context.blendFuncSeparate(srcRgb, dstRgb, srcAlpha, dstAlpha); } else { this.push(this.onBlendFuncSeparate, { x: srcRgb, y: dstRgb, z: srcAlpha, w: dstAlpha }); }
      };
      ...
      CommandBuffer.prototype.useProgram = function (program) {
        if (this.immediate) { this.context.useProgram(program == null ? null : program.getProgram()); } else { this.push(this.onUseProgram, { program }); }
      };
      ...
      CommandBuffer.prototype.setUniformParameter = function (uniform, parameter) {
        ...
        this.push(this.onSetUniformParameter, { uniform, commandBuffer: this, parameter, unit });
      };
      ...
      CommandBuffer.prototype.setUniform1F = function (uniform, x) {
        ...
        if (this.immediate) { this.context.uniform1f(uniform.getLocation(), x); } else { this.push(this.onSetUniform1F, { uniform, x }); }
      };
      ...
      CommandBuffer.prototype.setUniform1I = function (uniform, x) {
        if (uniform == null || uniform.getLocation() == null) { return; }
        if (this.immediate) { this.context.uniform1i(uniform.getLocation(), x); } else { this.push(this.onSetUniform1I, { uniform, x }); }
      };
      ...
      CommandBuffer.prototype.setUniform2F = function (uniform, x, y) {
        if (uniform == null || uniform.getLocation() == null) { return; }
        if (this.immediate) { this.context.uniform2f(uniform.getLocation(), x, y); } else { this.push(this.onSetUniform2F, { uniform, x, y }); }
      };
      ...
      CommandBuffer.prototype.setUniform3F = function (uniform, x, y, z) {
        if (uniform == null || uniform.getLocation() == null) { return; }
        if (this.immediate) { this.context.uniform3f(uniform.getLocation(), x, y, z); } else { this.push(this.onSetUniform3F, { uniform, x, y, z }); }
      };
      ...
      CommandBuffer.prototype.setUniform4F = function (uniform, x, y, z, w) {
        if (uniform == null || uniform.getLocation() == null) { return; }
        if (this.immediate) { this.context.uniform4f(uniform.getLocation(), x, y, z, w); } else { this.push(this.onSetUniform4F, { uniform, x, y, z, w }); }
      };
      ...
      CommandBuffer.prototype.setUniformTextureSize = function (uniform, tex) {
        ...
        } else { this.push(this.onSetUniformTextureSize, { uniform, texture: tex }); }
      };
      ...
      CommandBuffer.prototype.setUniformMatrix4 = function (uniform, transpose, values) {
        if (uniform == null || uniform.getLocation() == null) { return; }
        if (this.immediate) { this.context.uniformMatrix4fv(uniform.getLocation(), transpose, values); } else { this.push(this.onSetUniformMatrix4, { uniform, transpose, values }); }
      };
      ...
      CommandBuffer.prototype.bindBuffer = function (target, buffer) {
        ...
        if (this.immediate) { this.context.bindBuffer(target, buffer); } else { this.push(this.onBindBuffer, { target, buffer }); }
      };
      ...
      CommandBuffer.prototype.enableVertexAttribArray = function (index) {
        if (this.immediate) { this.context.enableVertexAttribArray(index); } else { this.push(this.onEnableVertexAttribArray, { index }); }
      };
      ...
      CommandBuffer.prototype.disableVertexAttribArray = function (index) {
        if (this.immediate) { this.context.disableVertexAttribArray(index); } else { this.push(this.onDisableVertexAttribArray, { index }); }
      };
      ...
      CommandBuffer.prototype.vertexAttribPointer = function (index, size, type, normalized, stride, offset) {
        if (this.immediate) { this.context.vertexAttribPointer(index, size, type, normalized, stride, offset); } else { this.push(this.onVertexAttribPointer, { index, size, type, normalized, stride, offset }); }
      };
      ...
      CommandBuffer.prototype.drawElements = function (mode, count, type, offset, elemSize) {
        ...
        this.drawCalls += 1;
        this.push(this.onDrawElements, { mode, count, type, offset });
      };
      ...
      CommandBuffer.prototype.bindFramebuffer = function (buffer, fitView) {
        if (this.immediate) { this.bindFramebufferInternal(buffer, fitView); } else { this.push(this.onBindFramebuffer, { commandBuffer: this, framebuffer: buffer, fitView }); }
      };
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    const Camera = (function (_super) {
      ...
      function Camera (game, near, far) {
        ...
        game.addDrawListInvalidationHandler(function (geom) {
          if (geom) { _this.invalidateGeometry(); }
      ...
      Camera.prototype.bufferShadowTargetBegin = function (buf, cascadeIndex) {
        ...
        const lightDirArray = buf.getArrayParameter(WebGame.Game.lightDirParam);
    ...
    const PerspectiveCamera = (function (_super) {
      __extends(PerspectiveCamera, _super);
      function PerspectiveCamera (game, fov, aspect, near, far) {
        const _this = _super.call(this, game, near, far) || this;
        _this.fov = fov;
        _this.aspect = aspect;
        return _this;
      }
    ...
    const OrthographicCamera = (function (_super) {
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const DrawableEntity = (function (_super) {
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const DebugLine = (function (_super) {
  ...
  (function (WebGame) {
    const DrawList = (function () {
      ...
      DrawList.prototype.buildHandleList = function (shaders) {
        ...
        const errorProgram = shaders.get(WebGame.Shaders.Error);
  ...
  (function (WebGame) {
    const Fog = (function () {
      ...
      Fog.prototype.populateCommandBufferParameters = function (buf) {
        ...
        const clipParams = buf.getArrayParameter(WebGame.Camera.clipInfoParam);
    ...
    const Game = (function () {
      function Game (container) {
        ...
        container.addEventListener('mousedown', function (evnt) {
          ...
          const handled = _this.onMouseDown(evnt.which, _this.getScreenPos(evnt.pageX, evnt.pageY, _this.mouseScreenPos), evnt.target);
        ...
        window.addEventListener('mouseup', function (evnt) {
          ...
          const handled = _this.onMouseUp(evnt.which, _this.getScreenPos(evnt.pageX, evnt.pageY, _this.mouseScreenPos), evnt.target);
          if (handled) { evnt.preventDefault(); }
          return handled;
        });
        ...
        window.addEventListener('mousewheel', function (evnt) {
          const handled = _this.onMouseScroll(evnt.wheelDelta / 400);
          if (handled) { evnt.preventDefault(); }
          return handled;
        });
        window.addEventListener('keydown', function (evnt) {
          ...
          let handled = _this.onKeyDown(evnt.which);
        ...
        window.addEventListener('keyup', function (evnt) {
          ...
          const handled = _this.onKeyUp(evnt.which);
          if (handled) { evnt.preventDefault(); }
          return handled;
        });
        ...
        this.animateCallback = function (time) {
          const deltaTime = time - _this.lastAnimateCallback;
      ...
      Game.prototype.getMouseScreenPos = function (out) {
        if (out == null) { out = new Facepunch.Vector2(); }
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    ...
    const Material = (function (_super) {
    ...
    var MaterialClone = (function (_super) {
      ...
      function MaterialClone (base, isDynamic) {
        const _this = _super.call(this, isDynamic) || this;
        ...
        if (base.program == null) {
          base.addOnLoadCallback(function (mat) {
            ...
            const thisProps = _this.properties;
            const thatProps = mat.properties;
            for (const prop in thatProps) {
    ...
    const MaterialLoadable = (function (_super) {
      __extends(MaterialLoadable, _super);
      function MaterialLoadable (game, url) {
        const _this = _super.call(this, false) || this;
      ...
      MaterialLoadable.prototype.addPropertyFromInfo = function (info) {
        switch (info.type) {
          ...
          case MaterialPropertyType.TextureUrl: {
            const texUrl = Facepunch.Http.getAbsUrl(info.value, this.url);
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const MaterialLoader = (function (_super) {
    ...
    const MeshGroup = (function () {
      ...
      MeshGroup.prototype.canAddMeshData = function (data) {
        ...
        for (let i = 0; i < this.attribs.length; ++i) {
          if (WebGame.VertexAttribute.compare(this.attribs[i], data.attributes[i]) !== 0) { return false; }
        }
  ...
  (function (WebGame) {
    const MeshManager = (function () {
      ...
      MeshManager.copyElement = function (src, dst, index) {
        ...
        for (let _i = 0, _a = dst.attributes; _i < _a.length; _i++) {
          const attrib = _a[_i];
      ...
      MeshManager.transform3F = function (data, attrib, action) {
        ...
        const vec = new Facepunch.Vector3();
      ...
      MeshManager.transform4F = function (data, attrib, action, defaultW) {
        ...
        const vertLength = MeshManager.getVertexLength(data.attributes);
        const normalized = attrib.normalized;
        const vec = new Facepunch.Vector4();
        if (attrib.size === 3) {
      ...
      MeshManager.prototype.addMeshData = function (data, getMaterial, target) {
        ...
        const newGroup = new WebGame.MeshGroup(this.context, data.attributes);
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    ...
    WebGame.Model = Model;
    const ModelLoadable = (function (_super) {
      __extends(ModelLoadable, _super);
      function ModelLoadable (game, url) {
        const _this = _super.call(this, game.meshes, game.materialLoader) || this;
        _this.loadProgress = 0;
        _this.url = url;
        return _this;
      }
      ...
      ModelLoadable.prototype.loadNext = function (callback) {
        ...
        Facepunch.Http.getJson(this.url, function (info) {
          ...
          for (let i = 0, iEnd = info.materials.length; i < iEnd; ++i) {
            const matUrl = Facepunch.Http.getAbsUrl(info.materials[i], _this.url);
            const mat = materials[i] = _this.materialLoader.load(matUrl);
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const ModelLoader = (function (_super) {
  ...
  (function (WebGame) {
    const ShaderManager = (function () {
      ...
      ShaderManager.prototype.getFromCtor = function (ctor) {
        ...
        this.ctorPrograms.push({ ctor, program });
        return program;
      };
      ...
      ShaderManager.prototype.dispose = function () {
        for (const name_4 in this.namedPrograms) {
  ...
  (function (WebGame) {
    const ShaderProgram = (function () {
      ...
      ShaderProgram.prototype.compile = function () {
        ...
        for (const name_5 in this.attribNames) {
    ...
    const BaseMaterialProps = (function () {
    ...
    const BaseShaderProgram = (function (_super) {
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    let Shaders;
    (function (Shaders) {
      const ComposeFrame = (function (_super) {
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    let Shaders;
    (function (Shaders) {
      const DebugLineProps = (function (_super) {
      ...
      const DebugLine = (function (_super) {
        ...
        function DebugLine (context) {
          const _this = _super.call(this, context, DebugLineProps) || this;
          const gl = context;
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    let Shaders;
    (function (Shaders) {
      const ModelBaseMaterialProps = (function (_super) {
      ...
      const ModelBase = (function (_super) {
        __extends(ModelBase, _super);
        function ModelBase (context, ctor) {
          const _this = _super.call(this, context, ctor) || this;
          const gl = context;
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    let Shaders;
    (function (Shaders) {
      const VertexLitGenericMaterialProps = (function (_super) {
      ...
      const VertexLitGeneric = (function (_super) {
        __extends(VertexLitGeneric, _super);
        function VertexLitGeneric (context) {
          const _this = _super.call(this, context, VertexLitGenericMaterialProps) || this;
          const gl = context;
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const ShadowCamera = (function (_super) {
      __extends(ShadowCamera, _super);
      function ShadowCamera (game, targetCamera) {
        const _this = _super.call(this, game, 1, 1, 0, 1) || this;
        _this.game = game;
      ...
      ShadowCamera.prototype.getFrustumBounds = function (near, far, bounds) {
        ...
        const perspCamera = this.targetCamera;
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const StaticProp = (function (_super) {
      __extends(StaticProp, _super);
      function StaticProp () {
        return _super.call(this, true) || this;
      }
      StaticProp.prototype.setColorTint = function (color) {
        if (this.tint != null) { this.tint.copy(color); } else { this.tint = new Facepunch.Vector3().copy(color); }
      };
      ...
      StaticProp.prototype.onModelLoaded = function (model) {
        ...
        this.drawable.clearMeshHandles();
        const meshData = WebGame.MeshManager.clone(model.getMeshData());
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    ...
    const RenderTexture = (function (_super) {
    ...
    const ProceduralTexture2D = (function (_super) {
      __extends(ProceduralTexture2D, _super);
      function ProceduralTexture2D (context, width, height, format, type) {
        const _this = _super.call(this, context, TextureTarget.Texture2D, format === undefined ? TextureFormat.Rgba : format, type === undefined ? TextureDataType.Uint8 : type, width, height) || this;
      ...
      ProceduralTexture2D.prototype.getPixel = function (x, y, target, dstIndex) {
        const pixels = this.pixels;
        if (target == null) { target = new Array(pixels.channels); }
    ...
    const TextureLoadable = (function (_super) {
      ...
      TextureLoadable.prototype.getLoadPriority = function () {
        if (_super.prototype.getLoadPriority.call(this) === 0) { return 0; }
      ...
      TextureLoadable.prototype.loadElement = function (element, value) {
        const target = Facepunch.WebGl.decodeConst(element.target != undefined ? element.target : this.info.target);
      ...
      TextureLoadable.prototype.loadNext = function (callback) {
        ...
        const url = Facepunch.Http.getAbsUrl(element.url, this.url);
...
(function (Facepunch) {
  let WebGame;
  (function (WebGame) {
    const TextureLoader = (function (_super) {
...
(function (Facepunch) {
  ...
  (function (WebGame) {
    ...
    const Uniform1F = (function (_super) {
    ...
    const Uniform1I = (function (_super) {
    ...
    const Uniform2F = (function (_super) {
    ...
    const Uniform3F = (function (_super) {
    ...
    const Uniform4F = (function (_super) {
    ...
    const UniformSampler = (function (_super) {
      __extends(UniformSampler, _super);
      function UniformSampler (program, name) {
        const _this = _super.call(this, program, name) || this;
    ...
    const UniformMatrix4 = (function (_super) {
