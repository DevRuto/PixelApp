...
(function (Gokz) {
  ...
  const SeekOrigin = Gokz.SeekOrigin;
...
(function (Gokz) {
  ...
  Gokz.Event = Event;
  const ChangedEvent = (function (_super) {
  ...
  const KeyDisplay = (function () {
    function KeyDisplay (viewer, container) {
      ...
      viewer.showKeyDisplayChanged.addListener(function (showKeyDisplay) {
        if (showKeyDisplay && viewer.cameraMode === SourceUtils.CameraMode.Fixed) { _this.show(); } else { _this.hide(); }
      });
      viewer.cameraModeChanged.addListener(function (cameraMode) {
        if (viewer.showKeyDisplay && cameraMode === SourceUtils.CameraMode.Fixed) { _this.show(); } else { _this.hide(); }
      });
    ...
    KeyDisplay.prototype.updateButtons = function (tickData) {
      for (const key in this.buttonMap) {
        const pressed = (tickData.buttons & parseInt(key)) !== 0;
        if (pressed) {
    ...
    KeyDisplay.prototype.updateSync = function () {
      ...
      this.syncValueElem.textContent = (syncFraction * 100).toFixed(1);
    };
    ...
    KeyDisplay.prototype.updateSpeed = function () {
      ...
      this.speedValueElem.textContent = speedString;
    };
  ...
  const OptionsMenu = (function () {
    function OptionsMenu (viewer, container) {
      ...
      viewer.showOptionsChanged.addListener(function (showOptions) {
        if (showOptions) { _this.show(); } else { _this.hide(); }
      });
    }
    ...
    OptionsMenu.prototype.showMainPage = function () {
      ...
      this.addToggleOption('Free Camera', function () { return viewer.cameraMode === SourceUtils.CameraMode.FreeCam; }, function (value) {
        return viewer.cameraMode = value
    ...
    OptionsMenu.prototype.addToggleOption = function (label, getter, setter, changed) {
      ...
      const toggle = option.getElementsByClassName('toggle')[0];
  ...
  const ReplayControls = (function () {
    function ReplayControls (viewer) {
      ...
      this.speedElem.addEventListener('click', function (ev) {
        if (_this.speedControlVisible) { _this.hideSpeedControl(); } else { _this.showSpeedControl(); }
      });
      ...
      viewer.updated.addListener(function (dt) {
        if ((viewer.isPlaying && !_this.mouseOverPlaybackBar) || viewer.isPointerLocked()) {
          const sinceLastAction = (performance.now() - _this.lastActionTime) / 1000;
          const hidePeriod = viewer.isPointerLocked() ? 0 : _this.autoHidePeriod;
...
(function (Gokz) {
  ...
  const EntityFlag = Gokz.EntityFlag;
...
(function (Gokz) {
   ...
   */
  const ReplayViewer = (function (_super) {
    ...
    function ReplayViewer (container) {
      ...
      this.isPlayingChanged.addListener(function (isPlaying) {
        if (!isPlaying && _this.saveTickInHash) { _this.updateTickHash(); }
        if (isPlaying) {
    ...
    ReplayViewer.prototype.onMouseUp = function (button, screenPos, target) {
      ...
      if (_super.prototype.onMouseUp.call(this, button, screenPos, target)) { return true; }
...
var Gokz;
(function (Gokz) {
  const RouteLine = (function (_super) {
    ...
    RouteLine.prototype.onPopulateDrawList = function (drawList, clusters) {
      for (let _i = 0, _a = this.segments; _i < _a.length; _i++) {
        const segment = _a[_i];
        if (clusters == null) {
        ...
        for (let _b = 0, clusters_1 = clusters; _b < clusters_1.length; _b++) {
          const cluster = clusters_1[_b];
